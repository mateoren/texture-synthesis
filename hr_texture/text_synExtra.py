...
def run_synthesis_pyramid(tex, images, proc_img, iterations, output_directory, weightsLayers, weightsPyramid, vgg_class=vgg19.Vgg19):
    ...
    with tf.Session(config=config) as sess:
        ...
        '''
        restrict0 = tf.maximum(0., tf.minimum(1., tex[0]))
        r_tex0 = tex[0].assign(restrict0)
        restrict1 = tf.maximum(0., tf.minimum(1., tex[1]))
        r_tex1 = tex[1].assign(restrict1)
        restrict2 = tf.maximum(0., tf.minimum(1., tex[2]))
        r_tex2 = tex[2].assign(restrict2)
        restrict3 = tf.maximum(0., tf.minimum(1., tex[3]))
        r_tex3 = tex[3].assign(restrict3)
        restrict4 = tf.maximum(0., tf.minimum(1., tex[4]))
        r_tex4 = tex[4].assign(restrict4)
        '''

        restrict = [tf.maximum(0., tf.minimum(1., tex[i])) for i in range(len(tex))]
        r_tex = [tex[i].assign(restrict[i]) for i in range(len(tex))]
...
def main():
    ...
    noises = [tf.Variable(i) for i in [j.reshape((1, np.shape(j)[0], np.shape(j)[1], 3)).astype("float32") for j in create_pyramids(init, args.pyramid)]]
