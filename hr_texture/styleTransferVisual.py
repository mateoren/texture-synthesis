...
def run_tensorflow(image, noiseImage, contentImage, output_directory, depth, weightsLayers, weightsLayersContent,
    ...
    with tf.Session() as sess:
        ...
        for hhh in range(0, len(fpassListNoise)):
            print('This is the size')
            print(fpassListNoise[0].conv_list[hhh].get_shape().as_list())
        tmp = fpassListNoise[0].conv_list[5].eval()
        tmp = tmp[:,:,:,0:3]
        tmp = tmp.reshape(np.shape(tmp)[1], np.shape(tmp)[2], 3)
        print(tmp)
        tf.summary.image('content', tmp, 3)
        ...
        for w in range(3, fpassListstyle[0].getLayersCount()):
            origin = fpassListstyle[0].conv_list[w]
            new = fpassListNoise[0].conv_list[w]
            shape = origin.get_shape().as_list()
            cosongo = origin.eval()
            print("hey man we have ")
            print(shape)
            print(cosongo.shape)
            N = shape[3]    #number of channels (filters)
            M = shape[1] * shape[2] #width x height
            #F = tf.reshape(origin, (-1, N), name='CreateF_style') #N x M
            tmpEE = origin[:,:,:,0:3].eval().reshape(shape[1], shape[2], 3)
            tmpEE = np.maximum(0, np.minimum(1, tmpEE))
            skimage.io.imsave(os.path.join(output_directory, "testPartial%s.png"%(str(w))), tmpEE)
